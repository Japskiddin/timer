...
//        }
    ...
    private void addShortcuts() {
        ...
        ShortcutInfo shortcutStopwatch = new ShortcutInfo.Builder(this, "idStopwatch")
                ...
                .setIntent(new Intent(getApplicationContext(), MainActivity.class).setAction(ACTION_STOPWATCH))
                .build();
    ...
    private void readStopwatchList() {
        ...
        if (tempList.size() == 0) {
            ...
            stopwatchList.add(new Stopwatch(String.format("%s %s", getResources().getString(R.string.card_note), stopwatchList.size() + 1), 0, 0, ITEM_TYPE_STOPWATCH, counter));
            counter++;
    ...
    private void addStopwatch() {
        int counter = SP.getInt(MainActivity.this, SP.ID, 0);
        Stopwatch stopwatchObject = new Stopwatch(String.format("%s %s", getResources().getString(R.string.card_note), stopwatchList.size() + 1), 0, 0, ITEM_TYPE_STOPWATCH, counter);
    ...
    @Override
    public void clickStopwatch(String type, int position) {
    ...
    private void readTimerList() {
        ...
        if (tempList.size() == 0) {
            int counter = SP.getInt(MainActivity.this, SP.ID, 0);
            timerList.add(new Stopwatch(String.format("%s %s", getResources().getString(R.string.app_name), timerList.size() + 1), 60000, 60000, ITEM_TYPE_TIMER, counter));
            counter++;
            SP.setInt(MainActivity.this, SP.ID, counter);
            saveToPref(SP.TIMER_LIST);
        } else {
    ...
    private void addTimer() {
        int counter = SP.getInt(MainActivity.this, SP.ID, 0);
        Stopwatch timerObject = new Stopwatch(String.format("%s %s", getResources().getString(R.string.app_name), timerList.size() + 1), 60000, 60000, ITEM_TYPE_TIMER, counter);
        counter++;
        SP.setInt(MainActivity.this, SP.ID, counter);
    ...
    @Override
    public void clickTimer(String type, int position) {
    ...
    @Override
    public void updateTime(int position, long millis) {
    ...
    protected void onStart() {
        ...
        if ((mNumberRunning == 5 && mAskRate && !mDontAsk)
                || (mNumberRunning == 15 && mAskRate && !mDontAsk)
                || (mNumberRunning == 25 && mAskRate && !mDontAsk)) {
        ...
        if ((mNumberRunning == 10 && mAskShare && !mDontAsk)
                || (mNumberRunning == 20 && mAskShare && !mDontAsk)
                || (mNumberRunning == 30 && mAskShare && !mDontAsk)) {
            AskDialog df = new AskDialog();
